...
void NetManager::MainLoop()
{
    ...
    while (mRunFlag)
    {
        ...
        const int ret = mPoll.Poll(mConnectionsCount + 1, mWaker.Sleep()?mTimeoutMs:0);
        mWaker.Wake();
        ...
        for (list<ITimeout *>::iterator it = mTimeoutHandlers.begin(); it != mTimeoutHandlers.end(); )
        {
            if (*it)
            {
        ...
        {
            KFS_LOG_STREAM_INFO << "timer overrun "
                                << (mNow - lastTimerTime)
                                << " seconds detected"
                                << KFS_LOG_EOM;
            mTimerOverrunCount++;
        ...
        while (slotCnt-- > 0)
        {
            ...
            while (mTimerWheelBucketItr != bucket.end())
            {
                ...
                assert(conn.IsGood());
                ++mTimerWheelBucketItr;
                NetConnection::NetManagerEntry& entry = *conn.GetNetManagerEntry();
                const int timeOut = conn.GetInactivityTimeout();
                if (timeOut < 0)
                {
                ...
                else
                {
                    ...
                    UpdateTimer(entry, slotCnt + int(entry.mExpirationTime - mNow));
                }
            }
    ...
    CheckFatalSysError(mPoll.Remove(mWaker.GetFd()), "failed to removed net kicker's fd from poll set");
    CleanUp();
}

void NetManager::CheckIfOverloaded()
{
...
inline const NetManager* NetManager::GetNetManager(const NetConnection& conn)
{
...
NetManager::Timer::Handler::Handler(NetManager& netManager,
                                    KfsCallbackObj& obj,
                                    int tmSec):
    KfsCallbackObj(),
    mObj(obj),
...
void NetManager::Timer::Handler::SetTimeout(int tmSec)
{
...
time_t NetManager::Timer::Handler::GetRemainingTime() const
{
...
int NetManager::Timer::Handler::EventHandler(int type, void* /* data */)
{
    switch (type)
    {
...
void NetManager::Timer::Handler::Cleanup()
{
    mConn->Close();
...
void NetManager::Timer::Handler::ResetTimeout()
{
...
void NetManager::Timer::Handler::ScheduleTimeoutNoLaterThanIn(int tmSec)
{
    if (tmSec < 0)
    {
        return;
