...
public:
    class OpOwner
    {
        // protected:
    public:
        virtual void OpDone(KfsOp* inOpPtr, bool inCanceledFlag, IOBuffer* inBufferPtr) = 0;
        virtual ~OpOwner() {}
        friend class Impl;
    };
    ...
    struct Stats
    {
        ...
        std::ostream& Display(std::ostream& inStream,
                             const char* inSeparatorPtr = 0,
                             const char* inDelimiterPtr = 0) const
        {
            const char* const theSeparatorPtr = inSeparatorPtr ? inSeparatorPtr : " ";
            const char* const theDelimiterPtr = inDelimiterPtr ? inDelimiterPtr : ": ";
            inStream <<
        ...
        Counter mConnectCount;      //链接次数
        Counter mConnectFailureCount;  //链接失败次数
        Counter mNetErrorCount; //网络出错次数
        Counter mConnectionIdleTimeoutCount; // 链接空闲超时次数
        Counter mResponseTimeoutCount; //响应超时次数
    ...
    bool Start(std::string inServerName,
               int inServerPort,
    ...
    bool SetServer(const ServerLocation& inLocation, bool inCancelPendingOpsFlag = true);
    void Stop();
    int GetMaxRetryCount() const;
    void SetMaxRetryCount(int inMaxRetryCount);
    int GetOpTimeoutSec() const;
    void SetOpTimeoutSec(int inTimeout);
    int GetIdleTimeoutSec() const;
    void SetIdleTimeoutSec(int inTimeout);
    int GetTimeSecBetweenRetries();
    void SetTimeSecBetweenRetries(int inTimeSec);
    ...
    void SetRetryConnectOnly(bool inFlag);
    void SetOpTimeout(int inOpTimeoutSec);
    void GetStats(Stats& outStats) const;
    bool Enqueue(KfsOp* inOpPtr, OpOwner* inOwnerPtr,IOBuffer* inBufferPtr = 0);
    bool Cancel(KfsOp* inOpPtr, OpOwner* inOwnerPtr);
    bool Cancel();
    std::string GetServerLocation() const;
    NetManager& GetNetManager();
    const NetManager& GetNetManager() const;
    void SetEventObserver(EventObserver* inEventObserverPtr); // Debug hook

private:
    Impl& mImpl;

private:
    KfsNetClient(const KfsNetClient& inClient);
    KfsNetClient& operator=(const KfsNetClient& inClient);
};
