...
typedef std::tr1::unordered_map<ChunkSpaceReservationKey_t,
                                size_t,
                                boost::hash<ChunkSpaceReservationKey_t> > ChunkSpaceResMap;
...
private:
    ...
    typedef std::list<OpPair,boost::fast_pool_allocator<OpPair> > PendingOpsList;
    ...
    template <typename T>
    bool GetWriteOp(T* wop, int align, IOBuffer *iobuf, int cmdLen, IOBuffer*& ioOpBuf);
