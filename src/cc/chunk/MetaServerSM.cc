...
time_t MetaServerSM::ConnectionUptime() const
{
    return (IsUp() ? (libkfsio::globalNetManager().Now() - mLastConnectTime) : 0);
}
...
int MetaServerSM::Connect()
{
    ...
    {
        KFS_LOG_STREAM_ERROR << "connection to meter server failed error: "
                             << QCUtils::SysError(-ret) << KFS_LOG_EOM;
        delete sock;
        return -1;
    }
    KFS_LOG_STREAM_INFO << (ret < 0 ? "connecting" : "connected")
...
int MetaServerSM::SendHello()
{
    ...
    if (! IsConnected())
    {
        KFS_LOG_STREAM_DEBUG << "unable to connect to meta server" << KFS_LOG_EOM;
        return -1;
    }
...
MetaServerSM::HandleRequest(int code, void *data)
{
    ...
    case EVENT_NET_READ:
    {
        ...
        {
            KFS_LOG_STREAM_ERROR << "exceeded max request header size: " << hdrsz
                                 << ">" << MAX_RPC_HEADER_LEN
                                 << " closing connection: " << (IsConnected() ?mNetConnection->GetPeerName() :string("not connected"))
                                 << KFS_LOG_EOM;
            iobuf->Clear();
