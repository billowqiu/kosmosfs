...
{
    return (mNetConnection ? mNetConnection->GetPeerName():std::string("not connected")
           );
}
...
ClientSM::HandleRequest(int code, void *data)
{
    ...
    case EVENT_NET_READ:
        if (IsWaiting())
        {
            CLIENT_SM_LOG_STREAM_DEBUG << "spurious read: "
                                       << (mCurOp ? mCurOp->Show() : "cmd")
                                       << " waiting for: "
                                       << GetByteCount()
                                       << " bytes of io buffers"
                                       << KFS_LOG_EOM;
...
ClientSM::OpFinished(KfsOp *doneOp)
{
    ...
    while (! mPendingOps.empty())
    {
        ...
        CLIENT_SM_LOG_STREAM_DEBUG << "submitting write-sync (" << p.dependentOp->seq
                                   << ") since " << p.op->seq << " finished" << KFS_LOG_EOM;
        mPendingSubmitQueue.splice(mPendingSubmitQueue.end(), mPendingOps, mPendingOps.begin());
    }
...
void ClientSM::ReleaseChunkSpaceReservations()
{
    for (ChunkSpaceResMap::iterator iter = mReservations.begin(); iter != mReservations.end(); iter++)
    {
        gAtomicRecordAppendManager.ChunkSpaceRelease(iter->first.chunkId, iter->first.transactionId, iter->second);
    }
}

RemoteSyncSMPtr ClientSM::FindServer(const ServerLocation &loc, bool connect)
{
    return KFS::FindServer(mRemoteSyncers, loc, connect);
}

void ClientSM::Granted(ClientSM::ByteCount byteCount)
{
    CLIENT_SM_LOG_STREAM_DEBUG << "granted: " << byteCount << " op: "
                               << (mCurOp ? mCurOp->Show() : std::string("null")) << KFS_LOG_EOM;
    if (! mNetConnection)
    {
        return;
    }
