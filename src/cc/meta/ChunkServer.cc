...
{
    sHeartbeatTimeout  = prop.getValue("metaServer.chunkServer.heartbeatTimeout", sHeartbeatTimeout);
    sHeartbeatInterval = max(3, prop.getValue("metaServer.chunkServer.heartbeatInterval", sHeartbeatInterval));
    sHeartbeatLogInterval = prop.getValue("metaServer.chunkServer.heartbeatLogInterval", sHeartbeatLogInterval);
}
...
ChunkServer::GetOp(IOBuffer& iobuf, int msgLen, const char* errMsgPrefix)
{
    ...
    const string loc = ServerID() + "/" + mNetConnection->GetPeerName();
    int maxLines = 64;
    ...
    {
        KFS_LOG_STREAM_ERROR << loc << " " << prefix << ": " << line << KFS_LOG_EOM;
    }
...
void ChunkServer::Heartbeat()
{
    ...
    if (mHeartbeatSent)
    {
        if (sHeartbeatTimeout >= 0 && timeSinceSent >= sHeartbeatTimeout)
        {
            ostringstream os;
            os << "heartbeat timed out, sent: " << timeSinceSent << " sec. ago";
            Error(os.str().c_str());
            return;
        }
...
ChunkServer::Dispatch()
{
    ...
    IOBuffer::OStream os;
    for (PendingReqs::Queue::const_iterator it = reqs.begin(); it != reqs.end(); ++it)
    {
        ...
        {
            KFS_LOG_STREAM_INFO << ServerID()
                                << " dispatched re-replication request: "" seq: "
                                << r.opSeqno << " "
                                << r.Show()
                                << KFS_LOG_EOM;
        }
