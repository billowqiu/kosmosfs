...
public:
    ...
    int Add(Fd inFd, int inOpType, void* inUserDataPtr)
    {
        if (inFd < 0)
        {
            return EBADF;
    ...
    int Set(Fd inFd, int inOpType, void* inUserDataPtr)
    {
        if (inFd < 0)
        {
            return EBADF;
    ...
    bool Next(int& outOpType, void*& outUserDataPtr)
    {
...
public:
    ...
    int Poll(int inMaxEventCountHint, int inWaitMilliSec)
    {
        ...
        const int theEventCount = inMaxEventCountHint > 1 ? inMaxEventCountHint : 1;
        ...
        mEpollEventCount = epoll_wait(mEpollFd, mEventsPtr, theEventCount, inWaitMilliSec);
        mNextEventIdx = 0;
...
private:
    ...
    int EPollEventMask(int inOpType)
    {
        int theRet = 0;
        if ((inOpType & kOpTypeIn) != 0)
        {
    ...
    int Ctl(int inEpollOp, Fd inFd, int inOpType, void* inUserDataPtr)
    {
        if (inFd < 0)
        {
            return EBADF;
        ...
        int ret = epoll_ctl(mEpollFd, inEpollOp, inFd, &theEpollEvent);
        return (ret ? errno : 0);
    }

private:
    Impl(const Impl& inImpl);
    Impl operator=(const Impl& inImpl);
};
...
private:
    ...
    void Alloc(Fd inFd, int inOpType, void* inUserDataPtr)
    {
        if (mFdCount >= mPollVecSize)
        {
            const int kQuantum = 256;
            struct pollfd* const thePtr = new struct pollfd[mPollVecSize + kQuantum];
    ...
    int PollEventMask(int inOpType)
    {
        int theRet = 0;
        if ((inOpType & kOpTypeIn) != 0)
        {
...
int
QCFdPoll::Poll(int inMaxEventCountHint, int inWaitMilliSec)
{
    return mImpl.Poll(inMaxEventCountHint, inWaitMilliSec);
}

bool QCFdPoll::Next(int& outOpType, void*& outUserDataPtr)
{
    return mImpl.Next(outOpType, outUserDataPtr);
}

int QCFdPoll::Remove(QCFdPoll::Fd inFd)
{
