...
QCThread::QCThread(QCRunnable* inRunnablePtr /* = 0 */, const char* inNamePtr /* = 0 */):
    mStartedFlag(false),
    mThread(),
...
int QCThread::TryToStart(QCRunnable* inRunnablePtr /* = 0 */, int inStackSize/* = -1 */,const char* inNamePtr/* = 0 */)
{
    if (mStartedFlag)
    {
        return EINVAL;
    ...
    if (inStackSize > 0 && (theErr = pthread_attr_setstacksize(&theStackSizeAttr, inStackSize)) != 0)
    {
    ...
    mStartedFlag = true;
    theErr = pthread_create(&mThread, &theStackSizeAttr, &Runner, mRunnablePtr);
    pthread_attr_destroy(&theStackSizeAttr);
    if (theErr != 0)
    {
        mStartedFlag = false;
...
void QCThread::FatalError(const char* inErrMsgPtr, int inSysError)
{
    QCUtils::FatalError(inErrMsgPtr, inSysError);
}

void* QCThread::Runner(void* inArgPtr)
{
    reinterpret_cast<QCRunnable*>(inArgPtr)->Run();
    return 0;
}

std::string QCThread::GetErrorMsg(int inErrorCode)
{
    return QCUtils::SysError(inErrorCode);
}

int QCThread::GetThreadCount()
{
