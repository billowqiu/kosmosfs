...
{
public:
    static void Init(NodeT& inNode)
    {
        inNode.mPrevPtr[ListT] = &inNode;
        inNode.mNextPtr[ListT] = &inNode;
    }

    static void Insert(NodeT& inNode, NodeT& inAfter)
    {
        if (&inNode == &inAfter)
        {
            return;
    ...
    static void Remove(NodeT& inNode)
    {
        inNode.mPrevPtr[ListT]->mNextPtr[ListT] = inNode.mNextPtr[ListT];
        inNode.mNextPtr[ListT]->mPrevPtr[ListT] = inNode.mPrevPtr[ListT];
        Init(inNode);
    ...
    static void Remove(NodeT& inStartNode, NodeT& inEndNode)
    {
    ...
    static bool IsInList(const NodeT& inNode)
    {
        return (inNode.mPrevPtr[ListT] != &inNode ||
                inNode.mNextPtr[ListT] != &inNode);
    }

    static NodeT& GetPrev(const NodeT& inNode)
    {
        return *inNode.mPrevPtr[ListT];
    }

    static NodeT& GetNext(const NodeT& inNode)
    {
        return *inNode.mNextPtr[ListT];
    }

    static NodeT* GetPrevPtr(const NodeT* inNodePtr)
    {
        return (inNodePtr ? inNodePtr->mPrevPtr[ListT] : 0);
    }

    static NodeT* GetNextPtr(const NodeT* inNodePtr)
    {
...
public:
    ...
    static void Init(NodeT** inListPtr)
    {
        inListPtr[ListT] = 0;
    }

    static bool IsEmpty(NodeT*const* inListPtr)
    {
        return (! inListPtr[ListT]);
    }

    static void PushFront(NodeT** inListPtr, NodeT& inNode)
    {
    ...
    static void PushBack(NodeT** inListPtr, NodeT&  inNode)
    {
        NodeT*& theHeadPtr = inListPtr[ListT];
        if (theHeadPtr)
        {
    ...
    static NodeT* PopFront(NodeT** inListPtr)
    {
        NodeT*&      theHeadPtr = inListPtr[ListT];
    ...
    static NodeT* PopBack(NodeT** inListPtr)
    {
        NodeT*& theHeadPtr = inListPtr[ListT];
    ...
    {
    public:
        Iterator(NodeT*const* inListPtr): 
        mHeadPtr(inListPtr),
