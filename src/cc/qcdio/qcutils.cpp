//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: qcutils.cpp 403 2010-11-30 00:27:26Z sriramsrao $
//
// Created 2008/11/01
//
...
/* static */ void
QCUtils::FatalError(const char* inMsgPtr, int inSysError)
{
    char      theMsgBuf[1<<9];
    const int theLen = DoSysErrorMsg(inMsgPtr, inSysError, theMsgBuf, sizeof(theMsgBuf));
...
/* static */ int
QCUtils::AllocateFileSpace(int inFd, int64_t inSize, int64_t inMinSize/*=-1*/, int64_t* inInitialFileSizePtr/*=0*/)
{
    int         theErr  = 0;
    ...
    int64_t*    inInitialFileSizePtr /* = 0 */)
{
    ...
    const int theErr = AllocateFileSpace(theFd, inSize, inMinSize, inInitialFileSizePtr);
