...
static int
GetAbsTimeout(QCMutex::Time inTimeoutNanoSec, struct timespec& outAbsTimeout)
{
...
bool QCMutex::Lock(QCMutex::Time inTimeoutNanoSec)
{
...
void QCMutex::RaiseError(const char* inMsgPtr, int inSysError)
{
...
bool QCCondVar::Wait(QCMutex& inMutex, QCMutex::Time inTimeoutNanoSec)
{
    struct timespec theAbsTimeout;
    int theErr = GetAbsTimeout(inTimeoutNanoSec, theAbsTimeout);
    if (theErr != 0)
...
void QCCondVar::RaiseError(const char* inMsgPtr, int inSysError)
{
    QCUtils::FatalError(inMsgPtr, inSysError);
}
